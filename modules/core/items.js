		...
		if (!error && response.statusCode == 200) {
			parseString(body, function (err, result) {
				if (result.items.results[0] > 0) {
					var found = '<center> <font color=#FFFF00> ::: ' + result.items.results[0] +  ' Results Found::: </font> </center> \n\n'
					...
					console.log('Item not found on cidb, searching local Db')
					localItemDb(userId,args)
				}	
			})	
		} else {
...
var localItemDb = function(userId, args) {
	connectdb().done(function(connection) {// Needs some work
		if (isNaN(args[0])) { searchText = args.join('%') } else { searchText = args.slice(1).join('%') }
		var searchText = searchText.replace(/exec|execute|select|insert|update|delete|create|alter|drop|rename|truncate|backup|restore|\*|\||\?/gim, '')
		query(connection,'SELECT * FROM aodb WHERE'  + (!isNaN(args[0]) ? ' highql >= ' + +args[0] + ' AND lowql <=' + args[0] + ' AND' : '') + ' name LIKE "' + searchText  + '%"' + 'OR name LIKE "% ' + searchText + '%"').done(function(result) {
			if(result[0].length !== 0 ) {
				var found = '<center> <font color=#FFFF00> ::: ' + result[0].length +  ' Results Found::: </font> </center> \n\n'
				found += 'Source: Local \n'
